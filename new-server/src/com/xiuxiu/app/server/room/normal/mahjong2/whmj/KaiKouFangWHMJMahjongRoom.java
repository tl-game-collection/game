...
@GameInfo(gameType = GameType.GAME_TYPE_WHMJ, gameSubType = 2)
public class KaiKouFangWHMJMahjongRoom extends WHMJMahjongRoom {
    // 杠类型, 1: 中发杠, 2: 中发白杠, 3: 中发皮杠
    protected int barType;

    public KaiKouFangWHMJMahjongRoom(RoomInfo info) {
        this(info, ERoomType.NORMAL);
    }

    public KaiKouFangWHMJMahjongRoom(RoomInfo info, ERoomType roomType) {
        super(info, roomType);
    }
    ...
    public void init() {
        ...
        this.barType = this.info.getRule().getOrDefault(RoomRule.RR_MJ_WH_BAR_TYPE, 1);
    }
    ...
    protected int getHuFang(IMahjongPlayer player) {
        ...
        fang += player.getScore(Score.MJ_CUR_TAKE_PI_CNT, false) + 2 * player.getScore(Score.MJ_CUR_TAKE_LAIZI_CNT, false);
