package com.xiuxiu.app.protocol.client.mahjong;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;

public class PCLIMahjongNtfDeskInfoByFZMJ extends PCLIMahjongNtfDeskInfo {
    public byte fangPai = -1;       // 翻的牌
    public byte laiZi = -1;         // 癞子的牌
    public List<Byte> piList = new ArrayList<>(); // 皮的牌列表
    public HashMap<Long,BuHuaInfo> buHuaInfo = new HashMap<>();

    public static class BuHuaInfo {
        public List<Byte> huaCard = new ArrayList<>();
        public int huaNUm;

        @Override
        public String toString() {
            return "BuHuaInfo{" +
                    "huaCard=" + huaCard +
                    ",huaNUm=" + huaNUm +
                    '}';
        }
    }

    @Override
    public String toString() {
        return "PCLIMahjongNtfDeskInfoByFZMJ{" +
                "fangPai=" + fangPai +
                ", buHuaInfo=" + buHuaInfo +
                ", laiZi=" + laiZi +
                ", piList=" + piList +
                ", remainCard=" + remainCard +
                ", other=" + other +
                ", allOnlineState=" + allOnlineState +
                ", crap1=" + crap1 +
                ", crap2=" + crap2 +
                ", fumbleOnBarCnt=" + fumbleOnBarCnt +
                ", roomInfo=" + roomInfo +
                ", bankerPlayerUid=" + bankerPlayerUid +
                ", bankerIndex=" + bankerIndex +
                ", curBureau=" + curBureau +
                ", gameing=" + gameing +
                '}';
    }
}
